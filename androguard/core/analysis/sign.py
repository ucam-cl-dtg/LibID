...
class Signature(object):
    def __init__(self, vmx):
        ...
        self.levels = {
                        ...
                        "L0" : {
                                    ...
                                    4 : ( "_get_strings_a",     "_get_fields_a",    "_get_packages_pa_3"),
                                },
    ...
    def _get_bb(self, analysis_method, functions, options, sdk_classes=[]):
        bbs = []
        xref_to = []
        ...
        for b in analysis_method.basic_blocks.get():
            ...
            for f in functions:
                try:
                    if f == "_get_packages_pa_3":
                        # print analysis_method, options
                        rst = self._get_packages_pa_3(analysis_method, options, sdk_classes)
                        internal.extend(rst[0])
                        if not xref_to:
                            xref_to.extend(rst[1])
                    else:
    ...
    # LibID
    def _get_packages_pa_3(self, analysis_method, include_packages, sdk_classes):
        key = "PA3-%s-%s" % (self._get_method_info(analysis_method), include_packages)
        if key in self._global_cached:
            return self._global_cached[ key ]
        ...
        for m in packages_method:
            for path in packages_method[ m ]:
                ...
                if path.get_access_flag() == 1:
                    ...
                    if present or (dst_class_name in sdk_classes):
                        l.append( (path.get_idx(), "M%s{%s%s%s}" % (PACKAGE_ACCESS[ path.get_access_flag() ], dst_class_name, dst_method_name, dst_descriptor ) ) )
                    else:
                        l.append( (path.get_idx(), "M%s" % (PACKAGE_ACCESS[ path.get_access_flag() ]) ) )
                        src_descriptor = analysis_method.get_method().get_descriptor()
                        xref_to.append("%s->%s->%s" % (src_descriptor, dst_class_name, dst_descriptor))
                else:
                    if present == True:
                        l.append( (path.get_idx(), "M%s{%s}" % (PACKAGE_ACCESS[ path.get_access_flag() ], m) ) )
                    else:
                        l.append( (path.get_idx(), "M%s" % (PACKAGE_ACCESS[ path.get_access_flag() ]) ) )

        self._global_cached[ key ] = (l, xref_to)

        return (l, xref_to)

    def get_method(self, analysis_method, signature_type, signature_arguments={}, sdk_classes=[]):
        ...
        #    print i, signature_arguments[ i ]
            if i == "L0":
                ...
                value, _xref_to = self._get_bb( analysis_method, _type, _arguments, sdk_classes )
                s.add( i, ''.join(z for z in value) )
                xref_to.extend(_xref_to)
                ...
                _type = ('_get_strings_a', '_get_fields_a', '_get_packages_pa_3')
